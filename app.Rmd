...
# Load task definitions with new unified loader
...
# Include CSS files in specific order for proper cascading
css_order <- c(
  "css/main.css",
  "css/header.css",
  "css/navbar.css",
...
```{r list_definitions}
# Simplified list metadata - using your color scheme
list_metadata <- list(
  list1 = list(
    id = "list1",
    name = "Lista I",
    ...
    div(
      ...
      div(
        ...
        actionButton(
          "toggle_sidebar",
          label = NULL,
          icon = icon(if(sidebar_collapsed()) "times" else "times"),
...
observe({
  lapply(names(list_metadata), function(list_id) {
    ...
    }, ignoreInit = TRUE, priority = 100)  # Higher priority = faster response
  })
})
...
output$list_sidebar <- renderUI({
  generate_list_sidebar(list_metadata, current_list(), all_lists)
})
