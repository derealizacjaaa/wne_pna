...
code_block <- function(code, language = "r", padding = "20px 60px") {
  ...
  if (is.list(output) && !is.null(names(output)) && "plots" %in% names(output)) {
    ...
    if (!is.null(output$text) && nzchar(output$text)) {
      content <- c(content, list(
        tags$pre(
          class = "code-output",
          style = "background: #ffffff; padding: 20px 60px; border: 1px solid #dee2e6; border-left: 4px solid #28a745; border-radius: 4px; overflow-x: auto; font-family: 'Courier New', monospace; font-size: 0.9em; margin: 15px 0; color: #333; white-space: pre;",
          output$text
        )
      ))
    }
