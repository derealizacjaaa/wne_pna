...
create_task_item <- function(task_id, task_info, is_active) {
  ...
  # Format task number: color first digit differently for double digits
  task_num <- task_info$task_num
  if (task_num < 10) {
    number_display <- sprintf("0%d", task_num)
  } else {
    first_digit <- substr(sprintf("%02d", task_num), 1, 1)
    second_digit <- substr(sprintf("%02d", task_num), 2, 2)
