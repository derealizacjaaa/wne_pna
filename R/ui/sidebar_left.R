...
# Fixed 5-slot layout with dynamic list discovery

#' Generate left sidebar with list selection and pagination
#'
#' Always shows 5 slots per page, fills empty slots with placeholders
#'
...
generate_list_sidebar <- function(list_metadata, current_list_id, all_lists,
  ...
  # Fixed configuration - always 5 slots per page
  slots_per_page <- 5
  ...
  # Build 5 slots (some may be empty)
  ...
  total_pages <- ceiling(total_lists / slots_per_page)
  if (total_pages == 0) total_pages <- 1  # At least 1 page even if empty
...
#' Create empty list slot placeholder
#' @return Shiny li tag
create_empty_list_slot <- function() {
  tags$li(
    class = "list-item list-item-empty",
    div(
      class = "list-item-content",
      div(class = "progress-bar-container-vertical",
          div(class = "progress-bar-fill-vertical", style = "height: 0%;")),
      div(class = "list-text",
          h4(style = "color: #ccc; opacity: 0.5;", "—")),
      div(class = "list-progress",
          tags$span(class = "progress-text empty", "—"))
    )
  )
}
